...
def procesar(csv_path: str, output_dir: str):
    ...
    for bid, registros in bancos_data.items():
        ...
        motor = cfg["motor"]
        if not registros:
            print(f"  ⚠️  Banco {bid} ({cfg['nombre']}): sin registros en el CSV")
            continue
        if motor == "postgres":
...
def _gen_mysql(bid, cfg, registros, now_str, output_dir):
    ...
    for chunk in chunks:
        rows = []
        for r in chunk:
            s = f"'{_escape_sql(r['saldo'])}'" if r["saldo_cifrado"] else f"'{r['saldo']}'"
            rows.append(
                f"  ('{_escape_sql(r['ci'])}', '{_escape_sql(r['nombres'])}', '{_escape_sql(r['apellidos'])}', "
                f"'{_escape_sql(r['nro_cuenta'])}', {r['banco_id']}, {s}, NULL, "
...
def _gen_mongo(bid, cfg, registros, now_str, output_dir):
    ...
    for chunk in chunks:
        docs = []
        for r in chunk:
            saldo_js = f'"{r["saldo"]}"' if r["saldo_cifrado"] else r["saldo"]
