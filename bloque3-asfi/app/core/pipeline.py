        ...
        async for batch in self.client.fetch_bank_batches(bank_id=bank_id, batch_size=batch_size, limit=limit):
            ...
            for account in batch.cuentas:
                ...
                try:
                    ...
                    decrypt_finished = perf_counter()
                    decrypt_time_ms = round((decrypt_finished - decrypt_started) * 1000, 3)

                    self.audit_logger.write(
                        {
                            "timestamp": utcnow_iso(),
                            "bank_id": bank_id,
                            "account_id": account.cuenta_id,
                            "event": "timing",
                            "batch_id": batch.lote_id,
                            "network_time_ms": batch.network_time_ms,
            ...
            if batch_conversions:
                ...
                sem = asyncio.Semaphore(settings.callback_concurrency)

                async def send_with_limit(item: ConversionRecord):
                    async with sem:
                        return await self.callback_service.send_result(item)

                callbacks = await asyncio.gather(
                    *(send_with_limit(item) for item in batch_conversions),
