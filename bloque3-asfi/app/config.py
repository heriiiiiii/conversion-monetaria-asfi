...
class Settings:
    ...
    rate_interval_seconds: float = float(os.getenv("ASFI_RATE_INTERVAL_SECONDS", "180"))
    ...
    bank_timeout_seconds: int = int(os.getenv("ASFI_BANK_TIMEOUT_SECONDS", "24"))
    processing_workers: int = int(os.getenv("ASFI_PROCESSING_WORKERS", "8"))

    callback_concurrency: int = int(os.getenv("ASFI_CALLBACK_CONCURRENCY", "25"))
    mysql_connect_retries: int = int(os.getenv("ASFI_MYSQL_CONNECT_RETRIES", "5"))
    mysql_connect_retry_delay: float = float(os.getenv("ASFI_MYSQL_CONNECT_RETRY_DELAY", "2"))
    mysql_connection_timeout: int = int(os.getenv("ASFI_MYSQL_CONNECTION_TIMEOUT", "24"))

    encrypt_large_amount_threshold: Decimal = Decimal(
        os.getenv("ASFI_ENCRYPT_LARGE_AMOUNT_THRESHOLD", "100000.0000")
    )
