...
class MariaDBAdapter(BaseDatabaseAdapter):
    ...
    def _ensure_connection(self) -> None:
        if self.connection is None or not self.connection.is_connected():
            if not self.connect():
                raise ConnectionError("No se pudo conectar a MariaDB")
    ...
    def get_cuentas(self, limit: int, offset: int) -> List[Dict[str, Any]]:
        ...
        return [self.normalize_cuenta(row) for row in rows]
    ...
    def get_cuenta_by_id(self, cuenta_id: CuentaId) -> Optional[Dict[str, Any]]:
        ...
        row = self.cursor.fetchone()
        return self.normalize_cuenta(row) if row else None
